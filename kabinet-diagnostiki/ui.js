...
const ICO = {
  ...
  user: '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.7"><circle cx="12" cy="8" r="3.2"/><path d="M5.5 19c1.2-3 3.4-4.5 6.5-4.5s5.3 1.5 6.5 4.5"/></svg>',
  ...
  bolt: '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8"><path d="M13 3 6.5 13h5L11 21l6.5-10h-5L13 3z"/></svg>'
};
...
function rxPanelHero(title, n, unitLine, desc, ico) {
  return `<div class="rx-panel__hero">
    ...
    <div class="rx-panel__hero-stat">
      <div class="rx-panel__stat-n"><b>${nfmt(n)}</b></div>
      <p class="rx-panel__stat-cap">${esc(unitLine)}</p>
    </div>
  </div>`;
}
...
function panelSpell(a) {
  ...
  return `${rxPanelNav('spell')}
    ${rxPanelHero(
      'Лишние формы записи',
      n,
      `${plural(n, 'запись найдена', 'записи найдено', 'записей найдено')}`,
    ...
    ${rxImpact([
      [ICO.warn, 'Можно переплатить за лицензии',
    ...
    <div class="rx-panel__main">
      <div class="rx-panel__sec-head">
        <h4>Найденные записи, где ПО записано разными способами</h4>
      </div>
...
function panelVendor(a) {
  ...
  return `${rxPanelNav('vendor')}
    ${rxPanelHero(
      ...
      n,
      `${plural(n, 'издатель найден', 'издателя найдено', 'издателей найдено')}`,
    ...
    <h4 class="rx-panel__sec-title">Чем это мешает бизнесу?</h4>
    ${rxImpact([
      [ICO.coin, 'Лицензии могут считаться неправильно',
        'Установки одного производителя попадают в разные группы. Часть может не попасть в расчет лицензий.'],
      [ICO.chart, 'Отчеты показывают неполную картину',
        'Данные по отечественному ПО и производителям будут неполными.']
    ])}
    <div class="rx-panel__main">
      <div class="rx-panel__sec-head">
...
function panelJunk(a) {
  ...
  return `${rxPanelNav('junk')}
    ${rxPanelHero(
      'Дефекты записей',
      n,
      `${plural(n, 'строка найдена', 'строки найдено', 'строк найдено')}`,
    ...
    ${rxImpact([
      ...
      [ICO.clock, 'Ручная обработка повторяется',
        `На разбор ${nfmt(n)} ${plural(n, 'строки', 'строки', 'строк')} уйдет около ${mins}. После следующей выгрузки такую работу придется повторять.`],
